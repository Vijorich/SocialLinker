...
- **Hover piano**: `assets/piano.js` plays a WebAudio-synthesized note when the pointer enters any `.link-card`/`.post-card` (loaded next to `badges.js` with `defer`). Card position in DOM order climbs an A-minor pentatonic scale across octaves (220 Hz base, 5 semitone steps per loop, so a sweep down or up the list plays a scale run); a per-card 1.2 s cooldown + a 55 ms global note gap keep sweeps musical without cutting notes mid-run. The AudioContext is created lazily on first hover so autoplay policy can't block it, and the module no-ops under `prefers-reduced-motion`. No-op on touch (no hover).
