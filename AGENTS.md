...
- **Hover piano**: `assets/piano.js` plays a WebAudio-synthesized note when the pointer enters any `.link-card`/`.post-card` (loaded next to `badges.js` with `defer`). Card position in DOM order walks an A-minor pentatonic set (440 Hz base, octave partial, 0.7 s decay); notes are capped at one at a time (350 ms window), the AudioContext is created lazily on first hover so autoplay policy can't block it, and the module no-ops under `prefers-reduced-motion`. No-op on touch (no hover).
