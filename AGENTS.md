...
- **Hover piano**: `assets/piano.js` plays a WebAudio-synthesized note when the pointer enters any `.link-card`/`.post-card` (loaded next to `badges.js` with `defer`). Card position in DOM order climbs an A-minor pentatonic scale across octaves (220 Hz base, 5 semitone steps per loop, so a sweep down or up the list plays a scale run); every card entry sounds with no throttle, so hovering up/down the list plays like an instrument. Hovering `.avatar` layers an ambient ember drone (three detuned sines at 110/165/220 Hz through a 900 Hz lowpass, gain breathing on a 7.5 s LFO in step with the CSS ember-breath animation, ~1.8 s release on leave, one drone at a time, tab-hide stops it). All AudioContexts are created lazily on first pointer interaction so autoplay policy can't block them, and the module no-ops under `prefers-reduced-motion`. No-op on touch (no hover).
