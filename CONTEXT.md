...
| `assets/post-modal.js` | `open(card)/swap(url,dir)/close()/popstate(state)/prefetch(url)` + window events `post-modal-open {url, source}` / `post-modal-close` | fetch+cache, generation token, dialog fill (h1→h2 demotion), scroll lock, View Transition morph, directional swap, history sync |
